...
async function callDb(action: string, payload: any) {
  const res = await authorizedFetch("/api/auth", {
    method: "POST",
...
export const upsertUserProfile = async (profile: UserProfile) => {
  ...
  // mutation (Convex rejects undeclared args). Only presentational fields are
  // sent, and `email` is overridden to the session token's email by the API proxy.
