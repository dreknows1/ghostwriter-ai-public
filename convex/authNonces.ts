...
/**
 * Single-use guard for the OAuth/Apple session-mint token (docs/PLAN.md "Auth on
 * iOS", SECURITY FIX). The signed token minted by api/oauth/callback.ts and
 * api/apple-verify.ts carries a random nonce; api/auth.ts calls `consume` before
 * minting a session.
 *
